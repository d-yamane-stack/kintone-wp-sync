...
/**
 * メール送信
 * @param {object} opts { subject, text, html, attachments, to }
 * @param {string} [opts.to] 宛先。未指定なら NOTIFY_EMAIL_TO を使う。
 */
async function sendMail(opts) {
  const info = await transporter.sendMail({
    from: `"RE-WRITE" <${process.env.NOTIFY_EMAIL_FROM}>`,
    to:   opts.to || process.env.NOTIFY_EMAIL_TO,
 ...
 * @param {string} [toEmail] サイト別アラート宛先（seo_site_configs.alertEmail）。
 *                           未指定なら NOTIFY_EMAIL_TO にフォールバックする。
 */
async function sendRankAlert(alerts, toEmail) {
...
`;
  ...
  await sendMail({ subject, text: rows, html, to: toEmail });
}
